...
export class EidRewriterSlaw {
  constructor () {
    ...
    this.dontPrefixUnlessAtt = ['section'];
  }
  ...
  rewriteElement (element, prefix) {
    ...
    if (element.tagName !== 'meta') {
      ...
      // only recalculates existing eIds instead of checking if the element should have one first
      if (oldEid) {
        // don't prefix e.g. section unless in an attachment
        if (this.dontPrefixUnlessAtt.includes(element.tagName) && !prefix.startsWith('att_') ) {
          prefix = '';
        }
  ...
  getEid (element, prefix) {
    ...
    if (!this.idUnnecessary.includes(name)) {
      ...
      // use e.g. subpart_nn for unnumbered elements that should have a num
      if (num === '' && this.numExpected.includes(name)) {
        num = 'nn';
        nn = true;
      }
